...
# 技能脚本 alias（需设置 SKILL_DIR 环境变量，脚本位于 dd-git-workflow/scripts 技能目录）
SKILL_DIR="$HOME/.trae-cn/skills/dd-git-workflow/scripts/scripts"
...
**脚本位置**：所有脚本位于 `scripts/` 子目录下（相对于本技能目录）。可直接执行 `bash <脚本路径>` 或通过 alias 调用。
