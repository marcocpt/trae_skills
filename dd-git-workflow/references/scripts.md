> 迁移来源：`dd-git-workflow/scripts/SKILL.md`。现作为按需 reference 使用，不参与顶层 Skill 路由。
...
本技能是 `invocation_mode=helper` 的原子能力：完成后返回调用方，不自行执行 Host Close。若它直接承接用户目标，顶层会话仍是 `standalone`，最终结束遵循 [dd-workflow-runtime/ask](../../dd-workflow-runtime/references/ask.md)。
...
| `scripts/cleanup-suggest.sh` | 废弃分支清理建议 | CleanupSuggestion JSON | dd-git-workflow/cleanup |
