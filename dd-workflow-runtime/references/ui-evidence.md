...
本技能固定为 `invocation_mode=helper`：返回 UI 证据与 Gate 结果，不自行 Host Close；顶层 `standalone` 会话按 [dd-workflow-runtime/ask](../../dd-workflow-runtime/references/ask.md) 收尾。
...
各 dd 技能中涉及 UI 的步骤引用本技能。引用格式：`UI 可观测性门禁遵循 [dd-workflow-runtime/ui-evidence](../../dd-workflow-runtime/references/ui-evidence.md)`
