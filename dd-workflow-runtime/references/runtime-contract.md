...
Codex 的 `native-agent` 还必须在派生前通过从 Skill 实际根目录调用的 `agents/check-review-route.py` 的 fail-closed 检查；provenance 与父 sandbox 判定细则以 [model-routing.md](model-routing.md) 为唯一属主，不在此复述。本文件只拥有执行侧后果：运行时把守卫 JSON 结果和 `sandbox_evidence` 持久化到 `routing` 证据，守卫非零退出或 `native_spawn_allowed=false` 时禁止派生原生 Reviewer；Generic Review Backend Router 不直接选择 `codex-native`，由检查通过且拥有 current-parent provenance 的 host-native dispatcher 接管（细则同上）；直接调用一个可写的 Reviewer 不构成 FR-008/DD-006 Gate 证据。
