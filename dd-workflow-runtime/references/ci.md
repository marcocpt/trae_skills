...
本技能固定为 `invocation_mode=helper`：只把 CI 状态、SHA、run、证据和 blocker 返回调用方，不自行输出最终摘要或 Host Close。直接承接用户目标时仍由顶层 `standalone` 会话按 [dd-workflow-runtime/ask](../../dd-workflow-runtime/references/ask.md) 收尾。
...
**语义**：合并产生新的 commit，必须验证合并后代码在 CI 中通过。按 `dd-workflow-runtime/test-location` skill 决策测试位置。
...
各 dd 工作流技能在涉及 CI 验证的步骤中引用本技能，替换重复的 CI 验证逻辑。引用格式：`CI 验证遵循 [dd-workflow-runtime/ci](../../dd-workflow-runtime/references/ci.md) 场景 <N>`
