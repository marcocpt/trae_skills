---
name: dd-shared-workflow-runtime
description: 当 dd 系列工作流需要跨 Trae/Codex 长时间稳定执行、跨会话恢复、从任意入口补齐依赖、控制 Stage Gate、按风险分配审查成本或在 Trae 完成后强制最终 ASK 时使用。供 project-bootstrap、feature-development、bug-fix 等编排型 Skill 调用；触发词：长流程运行时、workflow runtime、状态恢复、Gap Scan、Stage Contract、Trae 最终 ASK。
---
...
- 审查按 [dd-shared-subagent](../dd-shared-subagent/SKILL.md) 的 A/B/C 语义自检；命中高风险触发器时追加对应检查；检查范围和 CI 证据成本随之调整，但不删除验收条件或确定性验证。
