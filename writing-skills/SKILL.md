...
**内置前提：** 本技能自包含红-绿-重构：先运行无本技能的压力场景并记录失败（红），再修改 Skill 让场景通过（绿），最后用新的合理化压力场景堵漏洞（重构）。无需依赖外部 Superpowers 技能。
...
仅使用技能名称，带有明确的必需标记：
- ✅ 好的：`**必需规则：** 先建立失败基线，再修改并重跑压力场景`
- ✅ 好的：`**必需规则：** 先收集失败证据并验证单一假设，再修改 Skill`
- ❌ 差的：`参见某个外部 TDD Skill`（依赖不必要且不清楚是否可用）
- ❌ 差的：`@外部技能路径/SKILL.md`（强制加载，浪费上下文）
