...
8 步严格顺序工作流：需求确认 → 隔离环境 → TDD 调试 → 代码同步(含提交) → 启动app确认修复 → 文档检查 → Lint与Push → 合并清理。每步必须在前一步成功后才能继续。
...
digraph bug_fix_workflow {
    ...
    "3.2.5 CI 回归验证" -> "4. 启动app确认修复";
    "4. 启动app确认修复" -> "5. 文档检查" [label="已修复"];
    "4. 启动app确认修复" -> "2. TDD 调试修复" [label="未修复-继续TDD", style=dashed];
    "4. 启动app确认修复" -> "1. 创建工作树" [label="未修复-新工作树", style=dashed];
...
> **状态文件更新（HARD-GATE）**：进入步骤 4 前**必须**更新 `bug-fix-state.json` 的 `current_step="4"`。会话压缩后智能体凭此字段判断已进入「启动 app 确认修复」步骤，避免从步骤 1 重新验证 worktree。

---

## 步骤 4：启动 app 确认修复效果

完成一轮修复（步骤 3 代码同步 + CI 回归验证通过）后，**必须直接启动 app 让用户实际验证修复效果**，再询问是否已修复。禁止跳过启动 app 直接询问。
...
**启动失败处理** → **AskUserQuestion**：
- 选项 1（推荐）：重试启动（检查 scheme / 路径 / 依赖）
- 选项 2：跳过启动，直接询问修复结果（用户已自行验证）
- 选项 3：停止工作流

启动成功后告知用户：「应用已启动，请在应用中验证 bug 是否已修复。」

### 4.2 询问修复结果

等待用户在 app 中实际验证后，**AskUserQuestion**：
- 选项 1（推荐）：已修复，进入步骤 5 检查测试文档
- 选项 2：未修复，回到步骤 2 继续下一轮 TDD 调试修复（在同一工作树中继续）
- 选项 3：未修复，回到步骤 1 创建新工作树重新修复（当前工作树保留或清理由用户决定）

### 4.3 分支处理

- 选"已修复" → 更新 `current_step="5"`，进入步骤 5
- 选"未修复 - 回到步骤 2" → 更新 `current_step="2"`，跳转到步骤 2 重新执行，在同一工作树中继续下一轮 TDD，后续步骤顺序推进
- 选"未修复 - 回到步骤 1" → 更新 `current_step="1"`，跳转到步骤 1 重新执行，后续步骤顺序推进

> **状态文件更新（HARD-GATE）**：用户选择后**必须**立即更新 `bug-fix-state.json` 的 `current_step`。会话压缩后智能体凭此字段判断当前所处步骤，避免重复启动 app 或跳步。
...
**通过子代理分析**：`git diff` 输出 + 读需求文档/设计文档/测试用例表/代码，token 量大。调度子代理，传入 `BASE_BRANCH`、功能编号和文档路径，由子代理完成变更分析（5.2）、文档检查（5.3-5.6）和摘要输出（5.7）。
...
1. **需求文档**：`F{N}_*_需求文档.md`
2. **设计文档**：`F{N}_*_设计文档.md`
3. **测试用例表**：`F{N}_*_测试用例表.md`

使用 `rg --files docs/planning/P0/{功能编号}` 搜索 `*需求文档*.md`、`*设计文档*.md` 和 `*测试用例表*.md` 确认实际路径；如果 `rg` 不可用，使用 `find docs/planning/P0/{功能编号} -name '*需求文档*.md' -o -name '*设计文档*.md' -o -name '*测试用例表*.md'`。
...
### 5.4 检查需求文档/设计文档是否需要更新

对照最近代码变更，逐项检查需求文档和设计文档：

1. **验收标准（AC）**（需求文档）：新增或修改的行为是否需要新增/修改验收标准
2. **行为描述**（需求文档）：代码实现的行为是否与需求描述一致
3. **数据模型**（设计文档）：新增/修改的属性或方法是否需要在设计文档中记录
4. **状态机**（设计文档）：状态转换是否有变化
5. **当前问题**（需求文档）：已修复的问题是否需要从"当前问题"中移除

如果需要更新，直接修改需求文档/设计文档文件，并更新版本号和最后更新日期。

### 5.5 检查测试用例表是否需要更新

对照最近代码变更和需求文档/设计文档，逐项检查测试用例表：

1. **新增用例**：新增行为是否需要新增测试用例行
2. **状态更新**：已有用例的状态是否需要从 ❌/🟡 更新为 ✅
3. **现有证据**：新增的测试方法是否需要补充到"现有证据"列
4. **补充用例**：已实现的补充用例是否需要更新
5. **AC 映射**：新增用例是否需要关联到需求文档的验收标准编号
6. **统计更新**：更新文档头部的统计数字（总用例数、✅/🟡/❌/⏸️ 数量）

**测试用例表格式约定**：
- `#` 列：用例编号，保留历史编号；新增追加到对应分组末尾
- `状态` 列：✅ 已有测试能证明该行为；🟡 已有测试只覆盖部分断言；❌ 缺少可执行自动化；⏸️ 暂缓自动化
- `AC` 列：对应需求文档验收标准编号；`支撑` 表示不直接对应 AC
...
3. **文档版本号递增**：每次更新需求文档/设计文档或测试用例表时，必须递增版本号
