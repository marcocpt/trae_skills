...
1. 先原子写 `status=completed`，必要时写 Completion Receipt；
2. 禁止直接结束；
3. 结构化 ASK 只提供 `结束本次任务` / `还有其他任务`；
4. null 重问；“还有其他任务”启动新 Preflight；“结束本次任务”后输出最终摘要。
...
- 模式 B 未查真实代码就给一致性结论；
- 虚构文件、行号、修复文件或 SHA；
- 每轮省略可视化（纯参数/枚举文字问题除外）；
- 省略两阶段 ASK 或把沉默当作选择；
- TODO/LATER 不即时写入或未去重；
- 多方案未经用户选择就写入 TODO；
- 模式 B 有文件证据却按功能标签代替文件冲突分组；
- 未提交修复就标 `[x]`；
