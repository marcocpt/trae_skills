...
- 并行分组（按冲突最小化，供并行开发参考）   ← 收尾时生成，详见第 6 步第 3 项
  ...
  - 分组依据:: 模式 B 优先按「推荐修复文件」（无则 fallback「代码位置」）文件路径；模式 A 或无代码位置条目按功能标签（同标签串行、不同标签并行）
  - 并行建议:: P1+P2 共 5 条可由 2+ 开发者/代理同时进行；S1 内 3 条需串行
- [ ] TODO1. [P1] 问题简述 #登录
  - 文档位置:: [file.md#L10-20](file:///absolute/path/to/file.md#L10-20)
  - 现状:: 文档当前怎么写的
  - 改进建议:: 应该怎么改
  - 推荐修复文件:: [file.ts](file:///absolute/path/to/file.ts)（已有空桩，补全实现最合理）   ← 可选：模式 B 验证过代码时**必有**；模式 A 或无法推荐时省略整行
...
- [ ] TODO2. [P2] 另一个问题简述 #通用
  ...
  - 现状:: 另一个文档当前怎么写的
  - 改进建议:: 另一个应该怎么改
  - 推荐修复文件:: 新建 src/feature/new_module.py（功能完全不存在，需从零创建）
  - 审核时间:: YYYY-MM-DD HH:MM
```
...
| 推荐修复文件 | 可点击链接 `[文件名](file:///绝对路径)` + 推荐理由（括号内简述）。**模式 B 验证过代码时必有**（AI 分析问题后判断修复应放在哪个文件，可能与代码位置不同）；模式 A 或无法推荐时省略整行。若需新建文件则写「新建 路径」+ 理由 | 可选（模式 B 验证代码时**必有**） |
...
4. **创建并行分组并写入 TODO 文件**（按冲突最小化，供并行开发参考；空场景跳过）：
   - **分组依据**：
     - 模式 B：优先按各 TODO「推荐修复文件」字段（无则 fallback 到「代码位置」字段）的文件路径做**文件级冲突分析**。文件集合有交集的 TODO 归为同一「串行组」；无交集的归为「可并行批次」。**仅看文件路径，不看功能标签**（同功能标签但不同文件 = 可并行；同文件不同行 = 仍归串行组，因共享文件需串行修改）
...
4. **生成验证摘要文档**：为本批生成一份可手动验证的摘要文档，供用户按操作步骤逐条验证修复效果。
   ...
   - **格式**（markdown）：
     ...
     - 推荐修复文件:: [file.ts](file:///...)（理由）   ← 可选，沿用 TODO 条目中的推荐修复文件
...
- **TODO 缩进属性不全** → 3 必有（文档位置/现状/改进建议）+ 审核时间必有 + 1 可选（推荐修复文件：模式 B 验证过代码时必有，模式 A 或无法推荐时省略整行）+ 1 可选（代码位置：仅在实际引用代码锚点时添加，未引用则完全省略属性行）+ 1 可选（图形化预览：仅 HTML server 时添加）
- **模式 B TODO 条目缺少推荐修复文件**（验证过代码却未推荐修复应放在哪个文件）→ 模式 B 验证过代码时推荐修复文件字段**必有**；推荐理由必须简述（括号内），不得只写路径不给理由
...
- **模式 B 并行分组按功能标签而非文件路径**（有推荐修复文件/代码位置却按标签分组）→ 模式 B 必须优先按「推荐修复文件」（无则 fallback「代码位置」）文件路径做文件级冲突分析；同标签不同文件 = 可并行；仅模式 A 或无代码位置条目才按功能标签
...
| "按功能标签分组就够了，何必看文件" | 模式 B 有推荐修复文件/代码位置时必须按文件路径；同标签不同文件可并行，标签分组会误判为串行 |
| "推荐修复文件和代码位置一样，没必要重复" | 代码位置 = 已有代码在哪；推荐修复文件 = 修复应放在哪。两者可能不同（代码在 A 文件但修复应在 B 文件，或需新建文件），推荐修复文件是独立判断 |
| "推荐修复文件只是建议，不写也行" | 模式 B 验证过代码时推荐修复文件**必有**；不写则批量修复时 dd-bug-fix-workflow / dd-feature-development-workflow 需自行判断文件位置，缺少技能层面的推荐 |
| "推荐理由不用写，文件路径够了" | 无理由的推荐无判断依据，后续维护者无法理解为何推荐该文件；理由必须简述（括号内） |
...
- 模式 B TODO 含「推荐修复文件」+ 理由，批量修复时 dd-bug-fix-workflow / dd-feature-development-workflow 可直接获知修复应放在哪个文件，避免重复判断；并行分组优先按推荐修复文件做文件级冲突分析
