...
TODO 状态与验证摘要、逐批验证摘要属于 `working`/`evidence`，LATER 条目属于 `working`。详见 [dd-workflow-runtime/artifact-lifecycle](../../dd-workflow-runtime/references/artifact-lifecycle.md) §3。已关闭 finding 的逐批摘要只有在当前合同与必要证据已吸收后才可按 `retention` 清理；`unique_evidence` 需明确授权。LATER 的 evidence 仅指关闭后产生的独立验证附件，不指条目文件本身转 evidence。
