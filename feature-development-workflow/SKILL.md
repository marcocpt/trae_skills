...
- **发现必须修复的问题** → 逐条修复（或逐条用 `AskUserQuestion` 确认是否采纳修改建议），修复后**重新调度 3 个子代理并行检查**，确保修改未引入新问题
- **重试上限**：同一检查步骤并行检查重试次数上限为 3 次。第 3 次仍未通过 → `AskUserQuestion` 升级处理：继续重试 / 跳过该问题 / 停止工作流回退到上一步
- **逐条确认**：汇总后的每条修改建议需逐条用 `AskUserQuestion` 确认（即使有 10 条建议也一条一条问），不得批量默认采纳
...
- **通过但有建议** → 汇总建议，逐条用 `AskUserQuestion` 确认是否采纳；采纳后修改规范，并**重新调度 3 个子代理并行审核**，确保修改未引入新问题
- **发现必须修复的问题** → 逐条用 `AskUserQuestion` 确认修复方式，修复后**重新调度 3 个子代理并行审核**，直到通过或达到 3 次重试上限后升级处理
...
- **发现问题** → 逐条用 `AskUserQuestion` 确认修复方式，修正计划后**重新调度 3 个子代理并行核对**，直到通过或达到 3 次重试上限后升级处理
...
- **不通过** → 逐条用 `AskUserQuestion` 确认修复方式，修复问题后**重新调度 3 个子代理并行核对**，直到通过或达到 3 次重试上限后升级处理
