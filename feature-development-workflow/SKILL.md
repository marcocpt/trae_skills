...
if [ -f "$state_file" ]; then
    ...
    FEATURE_BRANCH=$(python3 -c "import json; print(json.load(open('$state_file'))['feature_branch'])")
    ...
    SPEC_PATH=$(python3 -c "import json; print(json.load(open('$state_file'))['spec_path'])")
    PLAN_DIR=$(python3 -c "import json; print(json.load(open('$state_file'))['plan_dir'])")
...
> **全局会话规则**：本工作流所有步骤中涉及用户决策的问题**都必须使用当前环境可用的结构化询问工具给出选项**。在 Trae 中使用 `AskUserQuestion`；在 Codex 中使用 `request_user_input`（如可用）或带清晰选项的简短文本问题。不得用无选项的纯文本提问中断会话。
...
1. **文档概述与项目背景**：做什么/不做什么
2. **用户角色与业务流程**（mermaid）
3. **功能需求详述**：按功能需求划分 Phase，每个 Phase 有目标、范围、交付物、验证方式
4. **数据需求与接口定义**
5. **非功能性需求**
6. **验收标准**：场景+预期+验证方式（必须有 XCTest 或 XCUITest 或对应测试框架的测试）
...
**UI 可观测性门禁**（涉及 UI 的特性必须通过）：

证据分层优先级从高到低：

1. **真实路径自动化证据**：E2E、XCUITest、Playwright、Appium，断言用户可见结果
2. **稳定可观测标记**：可访问性树、DOM、窗口层级、截图像素、canvas pixel、状态日志、activation marker、ready hook
3. **组件级 UI 证据**：渲染测试、快照、视觉回归、故事书截图
4. **手动验收证据**：明确步骤、预期画面、截图/录屏/日志路径
5. **内部状态证据**：单元测试、ViewModel 状态、Core 状态机、日志

**关闭规则**：

- 每个 UI AC 至少需要一种 1-4 层证据
- 只有第 5 层证据时，状态必须标为"未完成 UI 验证"
- 自动化不可行时，必须在设计规范和子计划中写明原因、手动验收步骤和剩余风险
- 任何"测试不到但应该没问题"的结论都必须升级为风险项
...
#### 5.4.3 生成 Conventional Commits 消息
...
先阅读 `docs/AI/trae-xctest-rules.md`（或 `docs/ai/trae-xctest-rules.md`），严格遵守其中的测试和回归规则。重点关注：

- 什么时候新增或更新回归测试
- 变更影响分析规范
- AI 输出格式

- **成功**：规则文件存在且已读取 → 进入 7.2
- **失败**：规则文件不存在 → 报错并停止，提示用户确认路径
- **边界**：规则文件存在但缺少关键章节 → 按现有内容执行，并在 7.7 自检中标注规则不完整
...
- **成功**：已获取变更并输出影响分析表
- **失败**：`git diff` 为空 → 提示"无代码变更，无需更新文档"，结束步骤 7

### 7.3 定位目标文档

根据步骤 0 确认的功能编号（如 F1.10），定位以下文档（路径模式为 `docs/planning/P0/{功能编号}/`）：
...
- **成功**：至少定位到一份目标文档 → 进入 7.4
- **失败**：功能编号对应目录不存在 → **AskUserQuestion**：
  - 选项 1（推荐）：新建目录
  - 选项 2：重新输入功能编号
  - 选项 3：终止
- **边界**：只找到一份文档 → 对缺失的另一份标注"文档不存在，跳过"
- **无法推断功能编号** → **AskUserQuestion**：询问用户功能编号
...
4. **状态机**：状态转换是否有变化
5. **当前问题**：已解决的问题是否需要从"当前问题"中移除

如果需要更新，直接修改设计规范文件，并更新版本号和最后更新日期。

### 7.5 检查测试用例表是否需要更新

对照代码变更和设计规范，逐项检查测试用例表：
...
- `状态` 列：✅ COVERED / 🟡 PARTIAL / ❌ MISSING / ⏸️ DEFERRED
...
> **调用上下文**：本工作流调用，步骤 4 的 TDD 已为新增行为写过测试，本步跳过"新增行为缺少测试覆盖"检查，仅保留以下三项检查。
...
3. 如果需要新增或修改测试代码，按照 `docs/AI/trae-xctest-rules.md` 的规范生成
...
1. **不得仅根据修改文件列表决定回归范围**：必须结合调用关系、数据流、共享模型、配置和用户流程判断
