...
4. **一次只问一个问题**——每个 `AskUserQuestion` 调用只包含一个问题，等人类回答后再问下一个。
5. **不得终止对话**——完成所有步骤后，必须用 `AskUserQuestion` 给出后续选项（如「查看重构报告 / 继续优化 / 终止本次重构」），禁止直接结束。
6. **给出代码链接位置**——每个问题必须附文件路径 + 行号，让人类能直接跳转查看。**提问前必须先核实代码位置**：用 Glob/Grep 确认文件存在、定位到具体行号，不得在未读文件的情况下凭用户描述附行号。若用户描述的文件名与代码不符（如用户说 `GridOverlayViewController` 但项目里实际是 `OverlayWindowController`），先 Glob/Grep 自查给出候选文件，在选项中让用户确认文件身份后再推进。若自查无任何候选文件，用 `AskUserQuestion` 给出选项：用户提供准确路径 / 先扫描相关模块列出文件清单 / 终止本次重构。用户拒绝确认文件身份时不得推进——必须再次确认，选项包含「提供准确路径让我核实 / 终止本次重构」，不得在未核实文件身份的情况下凭用户描述附行号或推进。
7. **能查官方资料的不问人类**——涉及 API 行为、系统约定、框架规范等可验证的事实，先开子代理查阅官方文档核对，记录结论即可。只有查不到权威依据的业务判断，才问人类。
8. **附查证结论摘要**——若提问前已有子代理查证结论，必须把结论摘要写进 `AskUserQuestion` 的问题上下文，让用户在知情下决策，避免信息不对称。
...
- **3 个子代理均通过且无问题** → 记录审核通过，**自动进入下一步**，无需 ASK
- **「必须修复」类** → 自动采纳并修复，无需 ASK；修复后**重新调度 3 个子代理并行检查**
- **「建议修复」/「可选优化」类** → 自动跳过并记录，无需 ASK（除非涉及重大架构变化，才用 `AskUserQuestion` 确认）
