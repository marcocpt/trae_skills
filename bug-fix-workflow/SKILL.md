...
6 步严格顺序工作流：需求确认 → 隔离环境 → TDD 调试 → 代码同步与文档检查 → 提交 → 合并清理。每步必须在前一步成功后才能继续。
...
digraph bug_fix_workflow {
    ...
    "0. 需求确认" -> "1. 创建工作树";
    "1. 创建工作树" -> "2. TDD 调试修复";
    "2. TDD 调试修复" -> "3. 代码同步+文档检查";
    "3. 代码同步+文档检查" -> "4. 提交 Git";
    "4. 提交 Git" -> "5. 合并清理";
...
> **全局会话规则**：本工作流所有步骤中涉及用户决策的问题（步骤 2 失败询问、步骤 3 变基选择/文档修复失败、步骤 4 失败、步骤 5 合并选择等）**都必须使用 `AskUserQuestion` 工具给出结构化选项**，不得用纯文本提问中断会话。
...
- 调用 `my-test-driven-debug` 分析并修复 bug（执行到第四步重构为止，**不包含 lint 检查与 git 提交**——这两项由本工作流步骤 4 统一执行，避免重复）
...
本步骤先拉取并变基到基线分支最新代码（确保在最终代码上检查文档），再同步测试文档。
...
- **先检测技能是否存在**：在工作目录的 `.trae/skills/my_test_doc_update/SKILL.md` 或全局技能目录中查找
  ...
  - **不存在**：跳过，输出提示"未找到 my_test_doc_update 技能，跳过测试文档同步"，继续步骤 4
...
- 步骤 4 lint 不通过时强行提交
- 没有失败测试就写修复
- 技能存在时跳过 3.3 不检查文档（技能不存在而跳过属于正常流程）
