...
##### Tunnel 工具路由锁定（提示词措辞，强制）

多轮强审里最高频的翻车是"审核方不调用 Tunnel——找不到工具、回退去猜绝对路径、或基于对话里我贴的上下文直接推断结论"。根因**不是仓库路径描述不清，而是工具发现与调用路径没有被锁死**。以下三规则用于任何需要 ChatGPT 读取本地仓库的请求（仓库整体审 / 复审 / 针对性审 / 决策建议审通用）。

1. **用"调用 Tunnel 工具"，不要只写"Tunnel 插件"**。"插件"不是稳定触发词；模型对工具名的联想可能是 `Tunnel`、`Tunnel.git_status`、`Tunnel.collect_review_evidence` 等变体。因此不要把某个具体子工具名当作**唯一**触发词；需要显式给出"第一步做什么"保证先调用工具时，可以在开场模板里写明具体调用（如下方固定模板中的 `Tunnel.git_status(...)`），但不要假设模型只认识某一个名字。
...
- 不允许猜绝对路径。
第一步只做：
Tunnel.git_status(repo="<repo>")
成功后继续读取文件并审查。
```

若当前会话没有可调用的 Tunnel 能力，或无法解析 repo，只回复一行：无法读取该仓库。
...
- 措辞未锁死 Tunnel 工具路由：用"Tunnel 插件"（非稳定触发词）、未要求"先调用工具再回答"、或未禁止基于上下文推断/猜绝对路径——三者任一缺失都可能让审核方拿不到仓库
