...
- 版本记录：仅保留最新一行（版本号 + 一句话语义变化），更早版本历史由 Git 承担；逐条复审修复细节（H1/M1/L1 修复过程、finding 闭环记录）落 `artifacts/` 的复审记录，不进正文。反模式：在正文逐版累积版本历史或修复过程，十余版累积上百行。
...
完成前自检：扫描 AC Then 是否复述 FR/RULE 细则、版本记录是否只保留最新一行、防御性文案是否只出现一处、追溯矩阵是否在正文手写、台账是否留在正文——任一命中，按本节规则收敛后重写，不修补。
