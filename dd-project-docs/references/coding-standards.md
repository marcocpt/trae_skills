> 迁移来源：`dd-project-docs/coding-standards/SKILL.md`。现作为按需 reference 使用，不参与顶层 Skill 路由。
...
**不适用：** 写需求文档（用 dd-writing-specs/requirements-writer）、写设计/架构契约（用 dd-writing-specs/design-writer / dd-write-architecture-constraint）、写 AGENTS.md / AI 协作约定（用 dd-project-docs/ai-conventions）、bug 修复、功能开发
...
digraph coding_standards_flow {
    ...
    review [label="审查\n(dd-workflow-runtime/review-gate)"];
    ask [label="结构化确认\n(dd-workflow-runtime/ask)"];
...
按 [dd-workflow-runtime/review-gate](../../dd-workflow-runtime/references/review-gate.md) 的通用 A/B/C 语义自检。编码规范附加检查：8 章节齐全、lint 配置存在且与规范一致、不混入架构契约/需求内容、命名约定前后一致、并发边界明确、错误处理策略明确、不与架构契约冲突、验证命令可执行、规则可机器检查且可人工检查。

### 结构化确认（复用 dd-workflow-runtime/ask）
...
null 输入按 dd-workflow-runtime/ask 规则重问，不得假设默认值。
...
digraph skill_relation {
    ...
    self [label="dd-project-docs/coding-standards\n(本 skill, 写法)", shape=box, style=filled, fillcolor=lightyellow];
    ai [label="dd-project-docs/ai-conventions\n(下游, AGENTS.md 引用规范)"];
...
- **下游**：dd-project-docs/ai-conventions 的 AGENTS.md 引用编码规范；功能开发时所有代码需遵守
- **流程 skill**：dd-project-bootstrap-workflow 的 Coding Standards 节点调度本 skill
- **共享**：复用 dd-workflow-runtime/review-gate（审查）、dd-workflow-runtime/ask（结构化询问 + null 重问）
