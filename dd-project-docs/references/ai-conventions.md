...
调用时声明 `invocation_mode=standalone|child`。`child` 只返回 AI 约定产物与 Gate，不执行 Host Close；`standalone` 按 [dd-workflow-runtime/ask](../../dd-workflow-runtime/references/ask.md) 收尾。
...
9. 按 [dd-workflow-runtime/review-gate](../../dd-workflow-runtime/references/review-gate.md) 审查；
...
按 [dd-workflow-runtime/review-gate](../../dd-workflow-runtime/references/review-gate.md) 的通用 A/B/C 语义自检。AI 协作入口附加检查：任务路由、必读入口、宿主范围完整；无重复 SSOT、路径存在、nested 作用域正确；命令可执行、禁止项可检查、Trae ASK 可触发。
...
- 上游：`dd-project-docs/architecture-contract`、`dd-project-docs/coding-standards`
- 编排：`dd-project-bootstrap-workflow` 的 AI Conventions 节点
- 下游：所有 Feature/Bug/Refactor 工作流和项目内 AI 代理
- 共享：`dd-workflow-runtime/ask`、`dd-workflow-runtime/review-gate`
