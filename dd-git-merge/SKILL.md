...
1. 先执行 `../dd-ai-git-workflow/scripts/pre-merge-check.sh` 自检，确认 `all_pass=true`
...
| ④ | AI 自检 | `../dd-ai-git-workflow/scripts/pre-merge-check.sh` | all_pass=false 禁止合并 |
...
- **禁止跳过 AI 自检直接合并**：合并前必须运行 `../dd-ai-git-workflow/scripts/pre-merge-check.sh`
