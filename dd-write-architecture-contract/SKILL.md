---
name: dd-write-architecture-contract
description: Use when 编写项目级全局架构契约与 ADR 索引，或在 AI Coding 场景下区分"路线级架构契约"与"功能级 Design 文档"。触发词：架构契约、全局架构契约、Architecture Contract、ADR 索引、ADR 流程、写架构、不变量、INV、禁止依赖方向、Public Compatibility Surface、allowlist、架构分区。症状：把项目级架构契约写成单个功能的设计文档、不变量里写"怎么实现"而非"什么永远成立"、把类名/方法签名当架构契约内容、brownfield 项目未列历史耦合点 allowlist、ADR 候选直接定稿未经评审批准、跳过 ADR 流程擅自修订不变量、把 UI 框架分区写进 Design 而非架构契约、依赖方向图与禁止依赖方向互相矛盾。
---

# 编写全局架构契约（dd-write-architecture-contract）
