...
| 子代理 | 审查维度 | 检查要点 |
|--------|---------|---------|
| **审查员 A** | 完整性 + 一致性 | TODO/占位符/不完整章节；需求、AC、Phase、测试策略是否互相冲突；内部矛盾 |
...
|------|---------|---------|---------|
| 视觉原型 | 与设计规范一致性 + 完整性 | 交互可操作性 + 边界状态 | UI 可观测性 + 与 AC 对应 |
| 测试用例表 | AC 覆盖完整性 + 编号规范 | 状态标注准确性 + 证据对应 | 与代码已有测试对照 + 缺失项识别 |
...
| 3. 3 子代理审 | 同一消息并行调度 3 个 | 提交审查结果文件 | 1 个代替 3 个、串行 |
