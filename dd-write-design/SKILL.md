---
name: dd-write-design
description: Use when 编写 02_Design.md 设计文档，或在 AI Coding 场景下区分 Design 与 Implementation Plan 文档层次。触发词：设计文档、Design、02_Design、写设计、架构契约。症状：Design 中出现类名/协议名/方法签名/字段类型/枚举值/文件路径/并发原语/框架 API/实现语言选型等 Implementation 细节，Design 复制 Requirements 内容而非引用 FR 编号，Design 讨论用户为什么需要而非由谁负责，把接口签名当 Design 层内容，团队把含完整代码的设计文档当 Design 用，用户问 Design 要不要写代码。
---
...
| 层次 | 回答 | 是否写代码 | 示例 |
|------|------|-----------|------|
| Requirements | 做什么 | ❌ 绝不 | "识别结束后展示层消失" |
| **Design** | **由谁负责、如何组织** | **❌ 不写代码符号** | **"识别模块负责生命周期，展示模块负责显示"** |
| Implementation Plan | 怎么实现（接口/类） | ✅ 可写接口签名、类定义 | `class RecognitionEngine` |
| Code | 具体实现 | ✅ 完整代码 | `func recognize() { ... }` |
...
- **Requirements**：用 dd-write-requirements，纯业务，无代码，产品合同
