...
digraph writing_specs {
    ...
    "5a. 设计文档（调 dd-write-design）" -> "5b. 视觉原型（涉及 UI 时）";
    "5b. 视觉原型（涉及 UI 时）" -> "5c. 测试用例表";
    "5c. 测试用例表" -> "结束" [label="所有文档完成"];
    "4. 合并总结 + 一次一问确认" -> "2. 写需求文档" [label="需修改", style=dashed];
}
```

<HARD-GATE>
严格按 0→1→2→3→4→5 顺序执行。步骤 4 确认通过后进入步骤 5，步骤 5 内部按"设计文档 → 视觉原型（涉及 UI 时）→ 测试用例表"顺序逐份完成（写 → 3 子代理审 → 合并 → 确认）。禁止跳步、禁止并行执行不同步骤、禁止跳过审查、禁止合并多文档一次确认。
