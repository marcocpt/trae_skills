...
**确认前置条件**：确认前必须把 3 子代理审查的合并总结完整展示给用户，用户未看到总结内容前不得发起确认询问。合并总结必须包含每位审查员的状态、问题列表（如有）和可选建议列表（如有），不得省略或压缩为总结性语句（如"三审均通过"）。
...
| "审查已通过且无必须修复问题，确认只是形式（rubber stamp）" | 确认的目的是让用户在知情下接受文档质量，不仅是 gate 下游。即使用户主动说"直接提交"，也必须用 AskUserQuestion 让用户显式选择。见 5.2.1。 |
| "用户明确说了'直接提交吧'，这是用户授权" | 用户口头指令 ≠ 知情下显式选择。必须用 AskUserQuestion 提供含拒绝项的选项，让用户在知情下决定。不得单方面解读为跳过授权。见 5.2.1。 |
| "最后一篇文档无下游，确认的 gate 功能无意义" | 确认不只为 gate 下游，还为让用户在知情下接受文档质量。最后一篇文档也必须确认。见 5.2.1。 |
| "三审已通过，质量已保证，确认是冗余的" | 审查是智能体间校验，确认是用户知情接受。两者目的不同，不可互相替代。 |
| "省交互轮次对用户友好，是 UX 优化" | 跳过确认剥夺了用户的知情决策权。"省交互"的代价是用户不知道自己接受了什么。见 5.2.1。 |
...
| 5a. 设计文档 | 子代理调用 dd-write-design，10 章节无占位符，遵守 P0 铁律 | 每篇确认后提交该文档+审查+总结 | 并行写多篇、跳过审查、跳过确认（见 5.2.1）、包含 AC/测试策略 |
| 5b. 视觉原型 | 涉及 UI 时，与设计文档一致 | 同上 | 跳过审查、跳过确认（见 5.2.1） |
| 5c. 测试用例表 | AC 覆盖完整，含测试策略/UI 可观测性矩阵 | 同上 | 跳过审查、跳过确认（见 5.2.1） |
